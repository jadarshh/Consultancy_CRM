...
// Safely format a date value (string | Date | null | undefined) to "YYYY-MM-DD" for date inputs
...
export default function EditStudentPage({
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/users?role=COUNSELOR").then((r) => r.json()),
    ])
      .then(([student, users]) => {
        if (Array.isArray(users)) setCounselors(users);
        if (student && student.id) {
          setForm({
            ...
            preferredLevel: student.preferredLevel ?? "",
            preferredField: student.preferredField ?? "",
            preferredIntake: student.preferredIntake ?? "",
            budgetMin: student.budgetMin != null ? String(student.budgetMin) : "",
            budgetMax: student.budgetMax != null ? String(student.budgetMax) : "",
            budgetCurrency: student.budgetCurrency ?? "USD",
            passportNumber: student.passportNumber ?? "",
            passportExpiryDate: toDateInput(student.passportExpiryDate),
            passportIssueDate: toDateInput(student.passportIssueDate),
            ...
            annualFamilyIncome:
              student.annualFamilyIncome != null
                ? String(student.annualFamilyIncome)
                : "",
            incomeCurrency: student.incomeCurrency ?? "USD",
            bankBalance:
              student.bankBalance != null ? String(student.bankBalance) : "",
            assignedCounselorId: student.assignedCounselorId ?? "",
          });
        } else {
